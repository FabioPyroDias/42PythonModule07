...
if __name__ == "__main__":
    ...
    try:
        ...
        card = EliteCard("Arcane Warrior", 9, Rarity.LEGENDARY.value,
                         5, 3, 10, "melee", 8)
        print()
        print("Combat phase:")
        enemy = CreatureCard("Enemy", 2, Rarity.COMMON.value, 2, 8)
        print(f"Attack result: {card.attack(enemy)}")
        print(f"Defense result: {card.defend(enemy.attack)}")
        print()
        print("Magic phase:")
        enemy1 = CreatureCard("Enemy1", 2, Rarity.COMMON.value, 2, 8)
        enemy2 = CreatureCard("Enemy2", 2, Rarity.COMMON.value, 2, 8)
